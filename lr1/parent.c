...
int main() {
    ...
    // Создаем pipe1 (для передачи результатов от ребенка к родителю)
    int pipe1[2];
    if (pipe(pipe1) == -1) {
    ...
    case 0: { // Дочерний процесс
        ...
        // Перенаправляем стандартный вывод в pipe1
        dup2(pipe1[1], STDOUT_FILENO);
        close(pipe1[1]);
        
        // Запускаем дочернюю программу с именем файла как аргумент
        char *args[] = {"child", filename, NULL};
        execv("./child", args);
        
        // Если дошли сюда, значит execv не удался
    ...
    default: { // Родительский процесс
        ...
        // Читаем из pipe1 и выводим в стандартный вывод
